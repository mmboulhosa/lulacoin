...
/* *** BEGIN DARKSEND MAGIC - LULA **********
    Copyright (c) 2014-2015, LulaCoin Developers
        eduffield - evan@lulacoinpay.io
        udjinm6   - udjinm6@lulacoinpay.io
*/
    ...
    if(fMasterNode) {
        ...
        BOOST_FOREACH(const CTransaction& txCollateral, vecSessionCollateral) {
            ...
            /*
                ...
                allow endless transaction that would bloat LulaCoin and make it unusable. To
...
bool CDarksendPool::DoAutomaticDenominating(bool fDryRun, bool ready)
{
    ...
    int64_t nBalanceNeedsAnonymized = nAnonymizeLulaCoinAmount*COIN - pwalletMain->GetAnonymizedBalance();
    ...
    if(!sessionFoundMasternode){
        ...
        double fLulaCoinSubmitted = (sessionTotalValue / CENT);
        LogPrintf("Submitting Darksend for %f LULA CENT - sessionTotalValue %d\n", fLulaCoinSubmitted, sessionTotalValue);
...
void ThreadCheckDarkSendPool()
{
    ...
    RenameThread("lulacoin-darksend");
        ...
        if(c % 60*(nLiquidityProvider+1) == 0){
            if(nLiquidityProvider!=0){
                ...
                if(nRand == 50+nLiquidityProvider && pwalletMain->GetAverageAnonymizedRounds() > 8){
                    ...
                    nAnonymizeLulaCoinAmount = (rand() % nLeftToAnon)+3;
                } else {
