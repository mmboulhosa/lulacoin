...
#if defined(NDEBUG)
# error "LulaCoin cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "LulaCoin Signed Message:\n";
...
static const int64_t nTargetTimespan = 24 * 60 * 60; // LulaCoin: 1 day
static const int64_t nTargetSpacing = 2.5 * 60; // LulaCoin: 2.5 minutes
...
unsigned int static DarkGravityWave(const CBlockIndex* pindexLast, const CBlockHeader *pblock) {
    /* current difficulty formula, lulacoin - DarkGravity v3, written by Evan Duffield - evan@lulacoinpay.io */
    const CBlockIndex *BlockLastSolved = pindexLast;
    const CBlockIndex *BlockReading = pindexLast;
...
unsigned int GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
        ...
        if (retarget == DIFF_BTC)
        {
            ...
            // LulaCoin: This fixes an issue where a 51% attack can change difficulty at will.
...
void ThreadScriptCheck() {
    RenameThread("lulacoin-scriptch");
