...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::LULA);
    ...
    if (!settings.contains("nAnonymizeLulaCoinAmount"))
        settings.setValue("nAnonymizeLulaCoinAmount", 1000);

    nDarksendRounds = settings.value("nDarksendRounds").toLongLong();
    nAnonymizeLulaCoinAmount = settings.value("nAnonymizeLulaCoinAmount").toLongLong();
    ...
    if (settings.contains("nAnonymizeLulaCoinAmount"))
        SoftSetArg("-anonymizelulacoinamount", settings.value("nAnonymizeLulaCoinAmount").toString().toStdString());
...
QVariant OptionsModel::data(const QModelIndex & index, int role) const
{
    if(role == Qt::EditRole)
    {
        ...
        case AnonymizeLulaCoinAmount:
            return QVariant(nAnonymizeLulaCoinAmount);
        default:
            return QVariant();
        }
    }
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case DarksendRounds:
            ...
            break;
        case AnonymizeLulaCoinAmount:
            nAnonymizeLulaCoinAmount = value.toInt();
            settings.setValue("nAnonymizeLulaCoinAmount", nAnonymizeLulaCoinAmount);
            emit anonymizeLulaCoinAmountChanged(nAnonymizeLulaCoinAmount);
            break;
        case CoinControlFeatures:
