...
int nAnonymizeLulaCoinAmount = 1000;
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\LulaCoin
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\LulaCoin
    // Mac: ~/Library/Application Support/LulaCoin
    // Unix: ~/.lulacoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "LulaCoin";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "lulacoin.conf"));
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty lulacoin.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override lulacoin.conf
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "lulacoind.pid"));
    ...
    if (vTimeOffsets.size() >= 5 && vTimeOffsets.size() % 2 == 1)
    {
        ...
        else
        {
            ...
            if (!fDone)
            {
                ...
                {
                    fDone = true;
                    string strMessage = _("Warning: Please check that your computer's date and time are correct! If your clock is wrong LulaCoin will not work properly.");
